...
function changeImage(direction) {
  ...
  if (direction === 1) { //avançar imagem
    if (index === imagens.length) index = 0;
  } else if (mudar === -1) { //retroceder imagem
    if (direction === -1) index = imagens.length-1;
  }
  document.getElementById('slide').src = servidorDasImagens+imagens[index].arquivo;
}

document.getElementById('proximo').addEventListener('click', function(e) {changeImage(1)});

document.getElementById('anterior').addEventListener('click', function(e) {changeImage(-1)});
